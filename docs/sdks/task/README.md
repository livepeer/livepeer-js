...
async function run() {
  ...
  const result = await sdk.task.getAll();
...
async function run() {
  ...
  const result = await sdk.task.get(taskId);
...
**Promise<[operations.GetTaskTaskIdResponse](../../models/operations/gettasktaskidresponse.md)>**
### Errors
