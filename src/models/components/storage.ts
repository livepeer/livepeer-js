/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { Spec, Spec$ } from "./spec";
import { z } from "zod";

export type One = {
    spec?: Spec | null | undefined;
};

/**
 * Set to true to make default export to IPFS. To customize the
 *
 * @remarks
 * pinned files, specify an object with a spec field. False or null
 * means to unpin from IPFS, but it's unsupported right now.
 *
 */
export type Ipfs = One | boolean;

export type Storage = {
    /**
     * Set to true to make default export to IPFS. To customize the
     *
     * @remarks
     * pinned files, specify an object with a spec field. False or null
     * means to unpin from IPFS, but it's unsupported right now.
     *
     */
    ipfs?: One | boolean | null | undefined;
};

/** @internal */
export namespace One$ {
    export type Inbound = {
        spec?: Spec$.Inbound | null | undefined;
    };

    export const inboundSchema: z.ZodType<One, z.ZodTypeDef, Inbound> = z
        .object({
            spec: z.nullable(Spec$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                ...(v.spec === undefined ? null : { spec: v.spec }),
            };
        });

    export type Outbound = {
        spec?: Spec$.Outbound | null | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, One> = z
        .object({
            spec: z.nullable(Spec$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                ...(v.spec === undefined ? null : { spec: v.spec }),
            };
        });
}

/** @internal */
export namespace Ipfs$ {
    export type Inbound = One$.Inbound | boolean;

    export type Outbound = One$.Outbound | boolean;

    export const inboundSchema: z.ZodType<Ipfs, z.ZodTypeDef, Inbound> = z.union([
        z.lazy(() => One$.inboundSchema),
        z.boolean(),
    ]);

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, Ipfs> = z.union([
        z.lazy(() => One$.outboundSchema),
        z.boolean(),
    ]);
}

/** @internal */
export namespace Storage$ {
    export type Inbound = {
        ipfs?: One$.Inbound | boolean | null | undefined;
    };

    export const inboundSchema: z.ZodType<Storage, z.ZodTypeDef, Inbound> = z
        .object({
            ipfs: z.nullable(z.union([z.lazy(() => One$.inboundSchema), z.boolean()])).optional(),
        })
        .transform((v) => {
            return {
                ...(v.ipfs === undefined ? null : { ipfs: v.ipfs }),
            };
        });

    export type Outbound = {
        ipfs?: One$.Outbound | boolean | null | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, Storage> = z
        .object({
            ipfs: z.nullable(z.union([z.lazy(() => One$.outboundSchema), z.boolean()])).optional(),
        })
        .transform((v) => {
            return {
                ...(v.ipfs === undefined ? null : { ipfs: v.ipfs }),
            };
        });
}
